...
class thx_csv_Dsv {
	...
	static function decode($dsv, $options) {
		...
		if($tmp3) {
			$dsv = thx_Strings::trimChars($dsv, "\x0A\x0D");
		}
		$result = null;
		if($dsv === "") {
	...
	static function requiresQuotes($value, $delimiter, $quote) {
		$tmp = null;
		$tmp1 = null;
		$tmp2 = !thx_Strings::contains($value, $delimiter);
		if($tmp2) {
			$tmp1 = thx_Strings::contains($value, $quote);
		} else {
			$tmp1 = true;
		}
		if(!$tmp1) {
			$tmp = thx_Strings::contains($value, "\x0A");
		} else {
			$tmp = true;
		}
		if(!$tmp) {
			return thx_Strings::contains($value, "\x0D");
		} else {
			return true;
		}
	}
