...
class thx_Strings {
	...
	static function capitalizeWords($value, $whiteSpaceOnly = null) {
		...
		if($whiteSpaceOnly) {
			...
			$tmp = thx_Strings::capitalize($value);
			return thx_Strings::$UCWORDS->map($tmp, (isset(thx_Strings::$upperMatch) ? thx_Strings::$upperMatch: array("thx_Strings", "upperMatch")));
		}
	}
	...
	static function caseInsensitiveCompare($a, $b) {
		...
		$tmp1 = strtolower($a);
		$tmp2 = strtolower($b);
		return thx_Strings::compare($tmp1, $tmp2);
	}
	...
	static function diffAt($a, $b) {
		$min = thx_Ints::min(strlen($a), strlen($b));
		{
			$_g1 = 0;
			while($_g1 < $min) {
				$i = $_g1++;
	...
	static function filterCharcode($s, $predicate) {
		$codes = thx_Strings::toCharcodes($s)->filter($predicate);
		return $codes->map(array(new _hx_lambda(array(), "thx_Strings_5"), 'execute'))->join("");
	}
	...
	static function humanize($s) {
		$tmp = thx_Strings::underscore($s);
		return StringTools::replace($tmp, "_", " ");
	}
	...
	static function randomSequence($seed, $length) {
		return thx_Ints::range(0, $length, null)->map(array(new _hx_lambda(array(&$seed), "thx_Strings_6"), 'execute'))->join("");
	}
	...
	static function startsWithAny($s, $values) {
		return thx_Iterables::any($values, array(new _hx_lambda(array(&$s), "thx_Strings_7"), 'execute'));
	}
	...
	static function toCharcodes($s) {
		return thx_Strings::map($s, array(new _hx_lambda(array(), "thx_Strings_8"), 'execute'));
	}
	...
	static function wrapColumns($s, $columns = null, $indent = null, $newline = null) {
		...
		return thx_Strings::$SPLIT_LINES->split($s)->map(array(new _hx_lambda(array(&$columns, &$indent, &$newline), "thx_Strings_9"), 'execute'))->join($newline);
	}
...
thx_Strings::$monoid = _hx_anonymous(array("zero" => "", "append" => array(new _hx_lambda(array(), "thx_Strings_10"), 'execute')));
...
function thx_Strings_5($i) {
	{
		return chr($i);
	}
}
function thx_Strings_6(&$seed, $_) {
	{
		return thx_Strings::random($seed, null);
	}
}
function thx_Strings_7(&$s, $start) {
	{
		return StringTools::startsWith($s, $start);
	}
}
function thx_Strings_8($s1) {
	{
		return haxe_Utf8::charCodeAt($s1, 0);
	}
}
function thx_Strings_9(&$columns, &$indent, &$newline, $part) {
	{
		$tmp = thx_Strings::$WSG->replace($part, " ");
		$tmp1 = StringTools::trim($tmp);
		return thx_Strings::wrapLine($tmp1, $columns, $indent, $newline);
	}
}
function thx_Strings_10($a, $b) {
	{
