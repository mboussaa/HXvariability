...
class thx__Path_Path_Impl_ {
	...
	static function base($this1, $end = null) {
		...
		$name = thx_Arrays::last($tmp1);
		$tmp2 = null;
		if(null !== $end) {
			$tmp2 = StringTools::endsWith($name, $end);
		} else {
			$tmp2 = false;
		}
		if($tmp2) {
			$tmp3 = strlen($name) - strlen($end);
			return _hx_substring($name, 0, $tmp3);
		}
		return $name;
	}
	static function ext($this1) {
		...
		$tmp1 = $this1->slice(2, null);
		return thx_Strings::afterLast(thx_Arrays::last($tmp1), ".");
	}
