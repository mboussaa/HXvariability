...
class thx_TestFunctions {
	...
	public function testAfter() {
		...
		while($_g1 < $_g) {
			...
			$manage = thx_Functions0::after($test, 3);
	...
	public function testOnce() {
		...
		while($_g1 < $_g) {
			++$_g1;
			$counter = 0;
			$test = array(new _hx_lambda(array(&$counter), "thx_TestFunctions_1"), 'execute');
			$manage = thx_Functions0::once($test);
	...
	public function testNegate() {
		...
		while($_g1 < $_g) {
			++$_g1;
			$tmp = thx_Functions0::negate(array(new _hx_lambda(array(), "thx_TestFunctions_3"), 'execute'));
			$tmp1 = call_user_func($tmp);
			utest_Assert::isFalse($tmp1, null, _hx_anonymous(array("fileName" => "TestFunctions.hx", "lineNumber" => 62, "className" => "thx.TestFunctions", "methodName" => "testNegate")));
			unset($tmp1,$tmp);
		}
	}
...
function thx_TestFunctions_1(&$counter) {
	{
		$tmp = ++$counter;
...
function thx_TestFunctions_2(&$counter, $x) {
	{
		++$counter;
		return $x * 10;
	}
}
function thx_TestFunctions_3() {
	{
		return true;
	}
}
