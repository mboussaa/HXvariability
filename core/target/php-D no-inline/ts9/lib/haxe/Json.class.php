...
class haxe_Json {
	...
	static function stringify($value, $replacer = null, $space = null) {
		return haxe_Json::phpJsonEncode($value, $replacer, $space);
	}
	...
	static function convertAfterDecode($val) {
		...
		$tmp = is_object($val);
		if($tmp) {
			$tmp1 = php_Lib::associativeArrayOfObject($val);
			$arr = haxe_Json::phpMapArray($tmp1, (isset(haxe_Json::$convertAfterDecode) ? haxe_Json::$convertAfterDecode: array("haxe_Json", "convertAfterDecode")));
			return _hx_anonymous($arr);
		} else {
			$tmp2 = is_array($val);
			if($tmp2) {
				$arr = haxe_Json::phpMapArray($val, (isset(haxe_Json::$convertAfterDecode) ? haxe_Json::$convertAfterDecode: array("haxe_Json", "convertAfterDecode")));
				return php_Lib::toHaxeArray($arr);
			} else {
				return $val;
			}
		}
	...
	static function convertBeforeEncode($val) {
		...
		return haxe_Json::phpMapArray($arr, (isset(haxe_Json::$convertBeforeEncode) ? haxe_Json::$convertBeforeEncode: array("haxe_Json", "convertBeforeEncode")));
	}
