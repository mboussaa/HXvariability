...
class thx_Iterators {
	...
	static function isIterator($v) {
		...
		$tmp2 = null;
		$tmp3 = !(!Lambda::has($fields, "next"));
		if($tmp3) {
			$tmp2 = !Lambda::has($fields, "hasNext");
		} else {
			$tmp2 = true;
		}
		if($tmp2) {
		...
		$tmp4 = Reflect::field($v, "next");
		$tmp5 = Reflect::isFunction($tmp4);
		if($tmp5) {
			$tmp6 = Reflect::field($v, "hasNext");
			return Reflect::isFunction($tmp6);
		} else {
			return false;
		}
	}
