...
class thx_Arrays {
	...
	static function compare($a, $b) {
		$v = thx_Ints::compare($a->length, $b->length);
		if($v !== 0) {
			return $v;
		}
		{
	...
	static function foldMap($array, $f, $m) {
		...
		$_e = $m;
		$tmp2 = array(new _hx_lambda(array(&$_e), "thx_Arrays_12"), 'execute');
		return thx_Arrays::foldLeft($tmp, $tmp1, $tmp2);
	}
	...
	static function sample($array, $n) {
		...
		$result = (new _hx_array(array()));
		{
			...
			while($_g1 < $_g) {
				++$_g1;
				$tmp = Std::random($copy->length);
				$tmp1 = $copy->splice($tmp, 1);
				$result->push($tmp1[0]);
				unset($tmp1,$tmp);
			}
		}
	...
	static function splitBy($array, $len) {
		...
		{
			$_g1 = 0;
			$tmp = $array->length / $len;
			$_g = Math::ceil($tmp);
			while($_g1 < $_g) {
				$p = $_g1++;
				$tmp1 = $array->slice($p * $len, ($p + 1) * $len);
				$res->push($tmp1);
				unset($tmp1,$p);
			}
		}
	...
	static function splitByPad($arr, $len, $pad) {
		...
		while(thx_Arrays::last($res)->length < $len) {
			thx_Arrays::last($res)->push($pad);
		}
		return $res;
	}
	...
	static function zip($array1, $array2) {
		$length = thx_Ints::min($array1->length, $array2->length);
		$array = (new _hx_array(array()));
		{
			$_g1 = 0;
			while($_g1 < $length) {
	...
	static function zipAp($ax, $fx) {
		...
		{
			$_g1 = 0;
			$_g = thx_Ints::min($ax->length, $fx->length);
			while($_g1 < $_g) {
				$i = $_g1++;
				$tmp = $fx[$i]($ax[$i]);
				$result->push($tmp);
	...
	static function maxBy($arr, $ord) {
		...
		if($tmp) {
			...
			$_e = $ord;
			$tmp1 = array(new _hx_lambda(array(&$_e), "thx_Arrays_21"), 'execute');
	...
	static function minBy($arr, $ord) {
		...
		if($tmp) {
			...
			$_e = $ord;
			$tmp1 = array(new _hx_lambda(array(&$_e), "thx_Arrays_22"), 'execute');
			$tmp2 = thx_Arrays::reduce($arr, $tmp1, $arr[0]);
			return haxe_ds_Option::Some($tmp2);
		}
	}
...
function thx_Arrays_14(&$f, $acc, $t) {
	{
		...
		$tmp2 = thx_Options::map($acc, array(new _hx_lambda(array(), "thx_Arrays_23"), 'execute'));
...
function thx_Arrays_15(&$f, &$s, $acc, $t) {
	{
		...
		$tmp1 = thx_Either::Right(array(new _hx_lambda(array(), "thx_Arrays_24"), 'execute'));
		$tmp2 = thx__Validation_Validation_Impl_::ap($acc, $tmp1, array(new _hx_lambda(array(), "thx_Arrays_25"), 'execute'));
...
function thx_Arrays_16(&$f, &$s, $acc, $t, $i) {
	{
		...
		$tmp1 = thx_Either::Right(array(new _hx_lambda(array(), "thx_Arrays_26"), 'execute'));
		$tmp2 = thx__Validation_Validation_Impl_::ap($acc, $tmp1, array(new _hx_lambda(array(), "thx_Arrays_27"), 'execute'));
		return thx__Validation_Validation_Impl_::ap($tmp, $tmp2, $s);
	}
}
...
function thx_Arrays_21(&$_e, $a0, $a1) {
	{
		return thx__Ord_Ord_Impl_::max($_e, $a0, $a1);
	}
}
function thx_Arrays_22(&$_e, $a0, $a1) {
	{
		return thx__Ord_Ord_Impl_::min($_e, $a0, $a1);
	}
}
function thx_Arrays_23($ux) {
	{
		return array(new _hx_lambda(array(&$ux), "thx_Arrays_28"), 'execute');
	}
}
function thx_Arrays_24($ux) {
	{
		return array(new _hx_lambda(array(&$ux), "thx_Arrays_29"), 'execute');
	}
}
function thx_Arrays_25($e1, $e2) {
	{
...
function thx_Arrays_26($ux) {
	{
		return array(new _hx_lambda(array(&$ux), "thx_Arrays_30"), 'execute');
	}
}
function thx_Arrays_27($e1, $e2) {
	{
		throw new HException("Unreachable");
	}
}
function thx_Arrays_28(&$ux, $u) {
	{
		$ux->push($u);
		return $ux;
	}
}
function thx_Arrays_29(&$ux, $u) {
	{
		$ux->push($u);
		return $ux;
	}
}
function thx_Arrays_30(&$ux, $u) {
	{
		$ux->push($u);
		return $ux;
	}
