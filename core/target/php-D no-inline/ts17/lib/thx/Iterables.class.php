...
class thx_Iterables {
	...
	static function findOption($it, $predicate) {
		$tmp = thx_Iterables::find($it, $predicate);
	...
	static function getOption($it, $index) {
		$tmp = thx_Iterables::get($it, $index);
		return thx_Options::ofValue($tmp);
	}
	...
	static function hasElements($it) {
		$tmp = $it->iterator();
		return thx_Iterators::hasElements($tmp);
	}
	...
	static function isEmpty($it) {
		$tmp = $it->iterator();
		return thx_Iterators::isEmpty($tmp);
	}
	static function isIterable($v) {
		$fields = null;
		$tmp = thx_Types::isAnonymousObject($v);
		if($tmp) {
			$fields = Reflect::fields($v);
		} else {
			$tmp1 = Type::getClass($v);
			$fields = Type::getInstanceFields($tmp1);
		}
		$tmp2 = !Lambda::has($fields, "iterator");
		if($tmp2) {
			return false;
		}
		$tmp3 = Reflect::field($v, "iterator");
		return Reflect::isFunction($tmp3);
	}
	...
	static function max($it, $ord) {
		$tmp = thx__Ord_Ord_Impl_::inverse($ord);
		return thx_Iterables::min($it, $tmp);
	}
	static function extremaBy($it, $f, $ord) {
		$found = haxe_ds_Option::$None;
		{
			...
			while(true) {
				...
				$a = $tmp->next();
				switch(Type::enumIndex($found)) {
				case 0:{
					$t = _hx_deref($found)->params[0];
					$tmp2 = call_user_func_array($f, array($a));
					$tmp3 = call_user_func_array($f, array($t->_0));
					$tmp4 = call_user_func_array($ord, array($tmp2, $tmp3));
					if($tmp4 === thx_OrderingImpl::$LT) {
						$found = haxe_ds_Option::Some(_hx_anonymous(array("_0" => $a, "_1" => $t->_1)));
					} else {
						$t1 = _hx_deref($found)->params[0];
						$tmp5 = call_user_func_array($f, array($a));
						$tmp6 = call_user_func_array($f, array($t1->_1));
						$tmp7 = call_user_func_array($ord, array($tmp5, $tmp6));
						if($tmp7 === thx_OrderingImpl::$GT) {
