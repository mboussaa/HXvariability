...
class thx_Objects {
	...
	static function combine($first, $second) {
		$to = _hx_anonymous(array());
		{
			...
			while($_g < $_g1->length) {
				$field = $_g1[$_g];
				++$_g;
				$tmp = Reflect::field($first, $field);
				Reflect::setField($to, $field, $tmp);
				unset($tmp,$field);
			}
		}
		{
			...
			while($_g2 < $_g11->length) {
				$field1 = $_g11[$_g2];
				++$_g2;
				$tmp1 = Reflect::field($second, $field1);
				Reflect::setField($to, $field1, $tmp1);
				unset($tmp1,$field1);
			}
		}
		return $to;
	...
	static function copyTo($src, $dst, $cloneInstances = null) {
		...
		}
		{
			...
			while($_g < $_g1->length) {
				...
				$tmp1 = null;
				$tmp2 = thx_Types::isAnonymousObject($sv);
				if($tmp2) {
					$tmp1 = thx_Types::isAnonymousObject($dv);
				} else {
					$tmp1 = false;
				}
				if($tmp1) {
				...
				unset($tmp2,$tmp1,$tmp,$sv,$field,$dv);
			}
		}
		return $dst;
	}
	...
	static function hasPath($o, $path) {
		...
		$current = $o;
		{
			...
			while($_g < $paths->length) {
				...
				++$_g;
				$tmp = thx_Strings::isDigitsOnly($currentPath);
				if($tmp) {
					...
					$tmp2 = Reflect::hasField($current, $currentPath);
					if($tmp2) {
		...
		$current = $o;
		{
			...
			while($_g < $paths->length) {
				...
				if($tmp) {
					...
					$tmp1 = Reflect::hasField($current, $currentPath);
					if($tmp1) {
						$current = Reflect::field($current, $currentPath);
					} else {
						return null;
	...
	static function getPathOption($o, $path) {
		$tmp = thx_Objects::getPath($o, $path);
		return thx_Options::ofValue($tmp);
	}
	...
	static function setPath($o, $path, $val) {
		...
		$current = $o;
		{
			...
			while($_g1 < $_g) {
				...
				$nextPath = $paths[$i + 1];
				$tmp = null;
				$tmp1 = !thx_Strings::isDigitsOnly($currentPath);
				if($tmp1) {
					$tmp = $currentPath === "*";
				} else {
					$tmp = true;
				...
				if($tmp) {
					...
					if($current[$index] === null) {
						$tmp2 = null;
						$tmp3 = !thx_Strings::isDigitsOnly($nextPath);
						if($tmp3) {
							$tmp2 = $nextPath === "*";
						} else {
							$tmp2 = true;
					...
					$tmp4 = !Reflect::hasField($current, $currentPath);
					if($tmp4) {
						$tmp5 = null;
						$tmp6 = !thx_Strings::isDigitsOnly($nextPath);
						if($tmp6) {
							$tmp5 = $nextPath === "*";
						} else {
							$tmp5 = true;
						...
						if($tmp5) {
							Reflect::setField($current, $currentPath, (new _hx_array(array())));
						} else {
							Reflect::setField($current, $currentPath, _hx_anonymous(array()));
						}
						unset($tmp6,$tmp5);
					}
					$current = Reflect::field($current, $currentPath);
		...
		$p = thx_Arrays::last($paths);
		$tmp7 = thx_Strings::isDigitsOnly($p);
		if($tmp7) {
...
function thx_Objects_5($existing, $nextPath) {
	{
		if($nextPath === "*") {
			...
			$tmp = thx_Strings::isDigitsOnly($nextPath);
			if($tmp) {
