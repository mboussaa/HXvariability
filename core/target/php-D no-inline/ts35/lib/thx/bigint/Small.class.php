...
class thx_bigint_Small implements thx_bigint_BigIntImpl{
	...
	public function divModSmall($small) {
		$tmp = $this->value / $small->value;
		$tmp1 = thx_Floats::trunc($tmp);
		$tmp2 = new thx_bigint_Small($tmp1);
		return _hx_anonymous(array("quotient" => $tmp2, "remainder" => new thx_bigint_Small(_hx_mod($this->value, $small->value))));
	}
	...
	public function multiplySmall($small) {
		...
		$tmp = thx_Ints::abs($small->value);
		$arr = thx_bigint_Bigs::smallToArray($tmp);
		$abs = thx_Ints::abs($this->value);
		if($abs < 10000000) {
	...
	public function shiftLeft($n) {
		...
		if(thx_Ints::abs($n) > 10000000) {
			$tmp = thx_bigint_Bigs::fromInt($n);
			$tmp1 = thx_bigint_Small::$two->pow($tmp);
			return $this->multiply($tmp1);
		}
		$result = $this;
