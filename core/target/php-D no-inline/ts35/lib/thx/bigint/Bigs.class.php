...
class thx_bigint_Bigs {
			...
			while($_g1 < $a_l) {
				...
				$a_i = $a[$i];
				{
					...
					while($_g3 < $b_l) {
						...
						$tmp = $i + $j;
						$tmp1 = thx_Floats::ftrunc($product - $carry * 10000000);
						$r[$tmp] = $tmp1;
						$r->a[$i + $j + 1] += $carry;
						unset($tmp1,$tmp,$j);
					}
	...
	static function multiplyKaratsuba($x, $y) {
		$n = thx_Ints::max($x->length, $y->length);
		if($n <= 400) {
		...
		$d = $y->slice($n, null);
		$c = $y->slice(0, $n);
		$ac = thx_bigint_Bigs::multiplyKaratsuba($a, $c);
		$bd = thx_bigint_Bigs::multiplyKaratsuba($b, $d);
		$tmp = thx_bigint_Bigs::addAny($a, $b);
	...
	static function divModSmall($value, $lambda) {
		...
		$tmp2 = thx_Floats::trunc($remainder);
