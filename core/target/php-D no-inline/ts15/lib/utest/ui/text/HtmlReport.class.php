	...
	public function addFixture($buf, $result, $name, $isOk) {
		...
		$messages = (new _hx_array(array()));
		{
			...
			while(true) {
				...
				switch(Type::enumIndex($assertation)) {
				case 0:{}break;
				case 1:{
					$pos = _hx_deref($assertation)->params[1];
					$msg = _hx_deref($assertation)->params[0];
					$tmp7 = "<strong>line " . _hx_string_rec($pos->lineNumber, "") . "</strong>: <em>";
					$tmp8 = StringTools::htmlEscape($msg, null);
					$messages->push(_hx_string_or_null($tmp7) . _hx_string_or_null($tmp8) . "</em>");
				}break;
				case 2:{
					$s = _hx_deref($assertation)->params[1];
					$e = _hx_deref($assertation)->params[0];
					$tmp9 = $this->getErrorDescription($e);
					$tmp10 = "<strong>error</strong>: <em>" . _hx_string_or_null($tmp9) . "</em>\x0A<br/><strong>stack</strong>:";
					$tmp11 = $this->getErrorStack($s, $e);
					$messages->push(_hx_string_or_null($tmp10) . _hx_string_or_null($tmp11));
				}break;
				case 3:{
					$s1 = _hx_deref($assertation)->params[1];
					$e1 = _hx_deref($assertation)->params[0];
					$tmp12 = $this->getErrorDescription($e1);
					$tmp13 = "<strong>setup error</strong>: " . _hx_string_or_null($tmp12) . "\x0A<br/><strong>stack</strong>:";
					$tmp14 = $this->getErrorStack($s1, $e1);
					$messages->push(_hx_string_or_null($tmp13) . _hx_string_or_null($tmp14));
				}break;
				case 4:{
					$s2 = _hx_deref($assertation)->params[1];
					$e2 = _hx_deref($assertation)->params[0];
					$tmp15 = $this->getErrorDescription($e2);
					$tmp16 = "<strong>tear-down error</strong>: " . _hx_string_or_null($tmp15) . "\x0A<br/><strong>stack</strong>:";
					$tmp17 = $this->getErrorStack($s2, $e2);
					$messages->push(_hx_string_or_null($tmp16) . _hx_string_or_null($tmp17));
				}break;
				case 5:{
				...
				case 6:{
					...
					$tmp18 = $this->getErrorDescription($e3);
					$tmp19 = "<strong>async error</strong>: " . _hx_string_or_null($tmp18) . "\x0A<br/><strong>stack</strong>:";
					$tmp20 = $this->getErrorStack($s3, $e3);
					$messages->push(_hx_string_or_null($tmp19) . _hx_string_or_null($tmp20));
				}break;
				case 7:{
					$tmp21 = StringTools::htmlEscape(_hx_deref($assertation)->params[0], null);
					$messages->push($tmp21);
				}break;
				}
				unset($tmp6,$assertation);
			}
		}
		$tmp22 = $messages->length > 0;
		if($tmp22) {
			$buf->add("<div class=\"testoutput\">");
			$tmp23 = $messages->join("<br/>");
			$buf->add($tmp23);
			$buf->add("</div>\x0A");
		}
		$buf->add("</div>\x0A");
							...
							while($_g4 < $_g5->length) {
								...
								$messages = "";
								{
									...
									while(true) {
										...
										case 1:{
											$buf->add("F");
											$tmp9 = call_user_func_array($indents, array(2));
											$messages .= _hx_string_or_null($tmp9) . "line: " . _hx_string_rec(_hx_deref($assertation)->params[1]->lineNumber, "") . ", " . _hx_string_or_null(_hx_deref($assertation)->params[0]) . _hx_string_or_null($newline);
										}break;
										case 2:{
											$buf->add("E");
											...
											$tmp13 = call_user_func_array($dumpStack, array(_hx_deref($assertation)->params[1]));
											$messages .= _hx_string_or_null($tmp12) . _hx_string_or_null($tmp13) . _hx_string_or_null($newline);
										}break;
										case 3:{
											$buf->add("S");
											$tmp14 = call_user_func_array($indents, array(2));
											$tmp15 = Std::string(_hx_deref($assertation)->params[0]);
											$tmp16 = _hx_string_or_null($tmp14) . _hx_string_or_null($tmp15);
											$tmp17 = call_user_func_array($dumpStack, array(_hx_deref($assertation)->params[1]));
											$messages .= _hx_string_or_null($tmp16) . _hx_string_or_null($tmp17) . _hx_string_or_null($newline);
										}break;
										case 4:{
											$buf->add("T");
											$tmp18 = call_user_func_array($indents, array(2));
											$tmp19 = Std::string(_hx_deref($assertation)->params[0]);
											$tmp20 = _hx_string_or_null($tmp18) . _hx_string_or_null($tmp19);
											$tmp21 = call_user_func_array($dumpStack, array(_hx_deref($assertation)->params[1]));
											$messages .= _hx_string_or_null($tmp20) . _hx_string_or_null($tmp21) . _hx_string_or_null($newline);
										}break;
										case 5:{
											$buf->add("O");
											$tmp22 = call_user_func_array($indents, array(2));
											$tmp23 = _hx_string_or_null($tmp22) . "missed async calls: " . _hx_string_rec(_hx_deref($assertation)->params[0], "");
											$tmp24 = call_user_func_array($dumpStack, array(_hx_deref($assertation)->params[1]));
											$messages .= _hx_string_or_null($tmp23) . _hx_string_or_null($tmp24) . _hx_string_or_null($newline);
										}break;
										case 6:{
											$buf->add("A");
											$tmp25 = call_user_func_array($indents, array(2));
											$tmp26 = Std::string(_hx_deref($assertation)->params[0]);
											$tmp27 = _hx_string_or_null($tmp25) . _hx_string_or_null($tmp26);
											$tmp28 = call_user_func_array($dumpStack, array(_hx_deref($assertation)->params[1]));
											$messages .= _hx_string_or_null($tmp27) . _hx_string_or_null($tmp28) . _hx_string_or_null($newline);
										}break;
										case 7:{
											$buf->add("W");
											$tmp29 = call_user_func_array($indents, array(2));
											$messages .= _hx_string_or_null($tmp29) . _hx_string_or_null(_hx_deref($assertation)->params[0]) . _hx_string_or_null($newline);
										}break;
										}
										unset($tmp8,$assertation);
									}
									unset($tmp7);
								}
								$buf->add($newline);
	...
	public function getTrace() {
		...
		$buf->add("<div class=\"trace\"><h2>traces</h2><ol>");
		{
			...
			while($_g < $_g1->length) {
				...
				$tmp1 = $this->formatStack($t->stack, false);
				$stack = StringTools::replace($tmp1, "'", "\\'");
				...
				$tmp2 = $this->formatTime($t->time);
				$buf->add("@ " . _hx_string_or_null($tmp2));
				$tmp3 = $t->delta * 1000;
				$tmp4 = Math::round($tmp3);
				if($tmp4 > 0) {
					$tmp5 = $this->formatTime($t->delta);
					$buf->add(", ~" . _hx_string_or_null($tmp5));
					unset($tmp5);
				}
				$buf->add("</span><span class=\"tracemsg\">");
				$tmp6 = StringTools::trim($t->msg);
				$tmp7 = StringTools::replace($tmp6, "\x0A", "<br/>\x0A");
				$buf->add($tmp7);
				$buf->add("</span><div class=\"clr\"></div></div></li>");
				unset($tmp7,$tmp6,$tmp4,$tmp3,$tmp2,$tmp1,$t,$stack,$method);
			}
		}
		$buf->add("</ol></div>");
		return $buf->toString();
