// Generated by Haxe 3.3.0
package haxe.io;

import haxe.root.*;

@SuppressWarnings(value={"rawtypes", "unchecked"})
public class Input extends haxe.lang.HxObject
{
	public Input(haxe.lang.EmptyObject empty)
	{
	}
	
	
	public Input()
	{
		//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
		haxe.io.Input.__hx_ctor_haxe_io_Input(this);
	}
	
	
	public static void __hx_ctor_haxe_io_Input(haxe.io.Input __temp_me35)
	{
	}
	
	
	public static java.lang.Object __hx_createEmpty()
	{
		//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
		return new haxe.io.Input(haxe.lang.EmptyObject.EMPTY);
	}
	
	
	public static java.lang.Object __hx_create(haxe.root.Array arr)
	{
		//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
		return new haxe.io.Input();
	}
	
	
	public boolean bigEndian;
	
	public java.nio.ByteBuffer helper;
	
	public int readByte()
	{
		//line 50 "/usr/lib/haxe/std/haxe/io/Input.hx"
		throw haxe.lang.HaxeException.wrap("Not implemented");
	}
	
	
	public int readBytes(haxe.io.Bytes s, int pos, int len)
	{
		//line 62 "/usr/lib/haxe/std/haxe/io/Input.hx"
		int k = len;
		//line 63 "/usr/lib/haxe/std/haxe/io/Input.hx"
		byte[] b = s.b;
		//line 64 "/usr/lib/haxe/std/haxe/io/Input.hx"
		if (( ( ( pos < 0 ) || ( len < 0 ) ) || ( ( pos + len ) > s.length ) )) 
		{
			//line 65 "/usr/lib/haxe/std/haxe/io/Input.hx"
			throw haxe.lang.HaxeException.wrap(haxe.io.Error.OutsideBounds);
		}
		
		//line 66 "/usr/lib/haxe/std/haxe/io/Input.hx"
		while (( k > 0 ))
		{
			//line 74 "/usr/lib/haxe/std/haxe/io/Input.hx"
			b[pos] = ((byte) (this.readByte()) );
			//line 76 "/usr/lib/haxe/std/haxe/io/Input.hx"
			 ++ pos;
			//line 77 "/usr/lib/haxe/std/haxe/io/Input.hx"
			 -- k;
		}
		
		//line 79 "/usr/lib/haxe/std/haxe/io/Input.hx"
		return len;
	}
	
	
	public void close()
	{
	}
	
	
	public boolean set_bigEndian(boolean b)
	{
		//line 91 "/usr/lib/haxe/std/haxe/io/Input.hx"
		this.bigEndian = b;
		//line 92 "/usr/lib/haxe/std/haxe/io/Input.hx"
		return b;
	}
	
	
	public haxe.io.Bytes readAll(java.lang.Object bufsize)
	{
		//line 104 "/usr/lib/haxe/std/haxe/io/Input.hx"
		if (haxe.lang.Runtime.eq(bufsize, null)) 
		{
			//line 108 "/usr/lib/haxe/std/haxe/io/Input.hx"
			bufsize = 16384;
		}
		
		//line 111 "/usr/lib/haxe/std/haxe/io/Input.hx"
		haxe.io.Bytes buf = haxe.io.Bytes.alloc(((int) (haxe.lang.Runtime.toInt(bufsize)) ));
		//line 112 "/usr/lib/haxe/std/haxe/io/Input.hx"
		haxe.io.BytesBuffer total = new haxe.io.BytesBuffer();
		//line 114 "/usr/lib/haxe/std/haxe/io/Input.hx"
		try 
		{
			//line 114 "/usr/lib/haxe/std/haxe/io/Input.hx"
			while (true)
			{
				//line 115 "/usr/lib/haxe/std/haxe/io/Input.hx"
				int len = this.readBytes(buf, 0, ((int) (haxe.lang.Runtime.toInt(bufsize)) ));
				//line 116 "/usr/lib/haxe/std/haxe/io/Input.hx"
				if (( len == 0 )) 
				{
					//line 117 "/usr/lib/haxe/std/haxe/io/Input.hx"
					throw haxe.lang.HaxeException.wrap(haxe.io.Error.Blocked);
				}
				
				//line 118 "/usr/lib/haxe/std/haxe/io/Input.hx"
				{
					//line 118 "/usr/lib/haxe/std/haxe/io/Input.hx"
					if (( ( len < 0 ) || ( len > buf.length ) )) 
					{
						//line 118 "/usr/lib/haxe/std/haxe/io/Input.hx"
						throw haxe.lang.HaxeException.wrap(haxe.io.Error.OutsideBounds);
					}
					
					//line 118 "/usr/lib/haxe/std/haxe/io/Input.hx"
					total.b.write(((byte[]) (buf.b) ), ((int) (0) ), ((int) (len) ));
				}
				
			}
			
		}
		catch (java.lang.Throwable __temp_catchallException1)
		{
			//line 114 "/usr/lib/haxe/std/haxe/io/Input.hx"
			haxe.lang.Exceptions.setException(__temp_catchallException1);
			//line 120 "/usr/lib/haxe/std/haxe/io/Input.hx"
			java.lang.Object __temp_catchall2 = __temp_catchallException1;
			//line 120 "/usr/lib/haxe/std/haxe/io/Input.hx"
			if (( __temp_catchall2 instanceof haxe.lang.HaxeException )) 
			{
				//line 120 "/usr/lib/haxe/std/haxe/io/Input.hx"
				__temp_catchall2 = ((haxe.lang.HaxeException) (__temp_catchallException1) ).obj;
			}
			
			//line 120 "/usr/lib/haxe/std/haxe/io/Input.hx"
			if (( __temp_catchall2 instanceof haxe.io.Eof )) 
			{
				//line 120 "/usr/lib/haxe/std/haxe/io/Input.hx"
				haxe.io.Eof e = ((haxe.io.Eof) (__temp_catchall2) );
				//line 120 "/usr/lib/haxe/std/haxe/io/Input.hx"
				{
				}
				
			}
			else
			{
				//line 120 "/usr/lib/haxe/std/haxe/io/Input.hx"
				throw haxe.lang.HaxeException.wrap(__temp_catchallException1);
			}
			
		}
		
		
		//line 122 "/usr/lib/haxe/std/haxe/io/Input.hx"
		return total.getBytes();
	}
	
	
	public void readFullBytes(haxe.io.Bytes s, int pos, int len)
	{
		//line 131 "/usr/lib/haxe/std/haxe/io/Input.hx"
		while (( len > 0 ))
		{
			//line 132 "/usr/lib/haxe/std/haxe/io/Input.hx"
			int k = this.readBytes(s, pos, len);
			//line 133 "/usr/lib/haxe/std/haxe/io/Input.hx"
			pos += k;
			//line 134 "/usr/lib/haxe/std/haxe/io/Input.hx"
			len -= k;
		}
		
	}
	
	
	public haxe.io.Bytes read(int nbytes)
	{
		//line 142 "/usr/lib/haxe/std/haxe/io/Input.hx"
		haxe.io.Bytes s = haxe.io.Bytes.alloc(nbytes);
		//line 143 "/usr/lib/haxe/std/haxe/io/Input.hx"
		int p = 0;
		//line 144 "/usr/lib/haxe/std/haxe/io/Input.hx"
		while (( nbytes > 0 ))
		{
			//line 145 "/usr/lib/haxe/std/haxe/io/Input.hx"
			int k = this.readBytes(s, p, nbytes);
			//line 146 "/usr/lib/haxe/std/haxe/io/Input.hx"
			if (( k == 0 )) 
			{
				//line 146 "/usr/lib/haxe/std/haxe/io/Input.hx"
				throw haxe.lang.HaxeException.wrap(haxe.io.Error.Blocked);
			}
			
			//line 147 "/usr/lib/haxe/std/haxe/io/Input.hx"
			p += k;
			//line 148 "/usr/lib/haxe/std/haxe/io/Input.hx"
			nbytes -= k;
		}
		
		//line 150 "/usr/lib/haxe/std/haxe/io/Input.hx"
		return s;
	}
	
	
	public java.lang.String readUntil(int end)
	{
		//line 159 "/usr/lib/haxe/std/haxe/io/Input.hx"
		haxe.root.StringBuf buf = new haxe.root.StringBuf();
		//line 160 "/usr/lib/haxe/std/haxe/io/Input.hx"
		int last = 0;
		//line 161 "/usr/lib/haxe/std/haxe/io/Input.hx"
		while (true)
		{
			//line 161 "/usr/lib/haxe/std/haxe/io/Input.hx"
			last = this.readByte();
			//line 161 "/usr/lib/haxe/std/haxe/io/Input.hx"
			if ( ! ((( last != end ))) ) 
			{
				//line 161 "/usr/lib/haxe/std/haxe/io/Input.hx"
				break;
			}
			
			//line 162 "/usr/lib/haxe/std/haxe/io/Input.hx"
			buf.addChar(last);
		}
		
		//line 163 "/usr/lib/haxe/std/haxe/io/Input.hx"
		return buf.toString();
	}
	
	
	public java.lang.String readLine()
	{
		//line 172 "/usr/lib/haxe/std/haxe/io/Input.hx"
		haxe.root.StringBuf buf = new haxe.root.StringBuf();
		//line 173 "/usr/lib/haxe/std/haxe/io/Input.hx"
		int last = 0;
		//line 174 "/usr/lib/haxe/std/haxe/io/Input.hx"
		java.lang.String s = null;
		//line 175 "/usr/lib/haxe/std/haxe/io/Input.hx"
		try 
		{
			//line 176 "/usr/lib/haxe/std/haxe/io/Input.hx"
			while (true)
			{
				//line 176 "/usr/lib/haxe/std/haxe/io/Input.hx"
				last = this.readByte();
				//line 176 "/usr/lib/haxe/std/haxe/io/Input.hx"
				if ( ! ((( last != 10 ))) ) 
				{
					//line 176 "/usr/lib/haxe/std/haxe/io/Input.hx"
					break;
				}
				
				//line 177 "/usr/lib/haxe/std/haxe/io/Input.hx"
				buf.addChar(last);
			}
			
			//line 178 "/usr/lib/haxe/std/haxe/io/Input.hx"
			s = buf.toString();
			//line 179 "/usr/lib/haxe/std/haxe/io/Input.hx"
			if (haxe.lang.Runtime.eq(haxe.lang.StringExt.charCodeAt(s, ( s.length() - 1 )), 13)) 
			{
				//line 179 "/usr/lib/haxe/std/haxe/io/Input.hx"
				s = haxe.lang.StringExt.substr(s, 0, -1);
			}
			
		}
		catch (java.lang.Throwable __temp_catchallException1)
		{
			//line 175 "/usr/lib/haxe/std/haxe/io/Input.hx"
			haxe.lang.Exceptions.setException(__temp_catchallException1);
			//line 180 "/usr/lib/haxe/std/haxe/io/Input.hx"
			java.lang.Object __temp_catchall2 = __temp_catchallException1;
			//line 180 "/usr/lib/haxe/std/haxe/io/Input.hx"
			if (( __temp_catchall2 instanceof haxe.lang.HaxeException )) 
			{
				//line 180 "/usr/lib/haxe/std/haxe/io/Input.hx"
				__temp_catchall2 = ((haxe.lang.HaxeException) (__temp_catchallException1) ).obj;
			}
			
			//line 180 "/usr/lib/haxe/std/haxe/io/Input.hx"
			if (( __temp_catchall2 instanceof haxe.io.Eof )) 
			{
				//line 180 "/usr/lib/haxe/std/haxe/io/Input.hx"
				haxe.io.Eof e = ((haxe.io.Eof) (__temp_catchall2) );
				//line 180 "/usr/lib/haxe/std/haxe/io/Input.hx"
				{
					//line 181 "/usr/lib/haxe/std/haxe/io/Input.hx"
					s = buf.toString();
					//line 182 "/usr/lib/haxe/std/haxe/io/Input.hx"
					if (( s.length() == 0 )) 
					{
						//line 183 "/usr/lib/haxe/std/haxe/io/Input.hx"
						throw haxe.lang.HaxeException.wrap(e);
					}
					
				}
				
			}
			else
			{
				//line 180 "/usr/lib/haxe/std/haxe/io/Input.hx"
				throw haxe.lang.HaxeException.wrap(__temp_catchallException1);
			}
			
		}
		
		
		//line 185 "/usr/lib/haxe/std/haxe/io/Input.hx"
		return s;
	}
	
	
	public final double readFloat()
	{
		//line 194 "/usr/lib/haxe/std/haxe/io/Input.hx"
		return haxe.io.FPHelper.i32ToFloat(this.readInt32());
	}
	
	
	public double readDouble()
	{
		//line 203 "/usr/lib/haxe/std/haxe/io/Input.hx"
		int i1 = this.readInt32();
		//line 204 "/usr/lib/haxe/std/haxe/io/Input.hx"
		int i2 = this.readInt32();
		//line 205 "/usr/lib/haxe/std/haxe/io/Input.hx"
		if (this.bigEndian) 
		{
			//line 205 "/usr/lib/haxe/std/haxe/io/Input.hx"
			return haxe.io.FPHelper.i64ToDouble(i2, i1);
		}
		else
		{
			//line 205 "/usr/lib/haxe/std/haxe/io/Input.hx"
			return haxe.io.FPHelper.i64ToDouble(i1, i2);
		}
		
	}
	
	
	public int readInt8()
	{
		//line 212 "/usr/lib/haxe/std/haxe/io/Input.hx"
		int n = this.readByte();
		//line 213 "/usr/lib/haxe/std/haxe/io/Input.hx"
		if (( n >= 128 )) 
		{
			//line 214 "/usr/lib/haxe/std/haxe/io/Input.hx"
			return ( n - 256 );
		}
		
		//line 215 "/usr/lib/haxe/std/haxe/io/Input.hx"
		return n;
	}
	
	
	public int readInt16()
	{
		//line 224 "/usr/lib/haxe/std/haxe/io/Input.hx"
		int ch1 = this.readByte();
		//line 225 "/usr/lib/haxe/std/haxe/io/Input.hx"
		int ch2 = this.readByte();
		//line 226 "/usr/lib/haxe/std/haxe/io/Input.hx"
		int n = ( (this.bigEndian) ? (( ch2 | ( ch1 << 8 ) )) : (( ch1 | ( ch2 << 8 ) )) );
		//line 227 "/usr/lib/haxe/std/haxe/io/Input.hx"
		if (( (( n & 32768 )) != 0 )) 
		{
			//line 228 "/usr/lib/haxe/std/haxe/io/Input.hx"
			return ( n - 65536 );
		}
		
		//line 229 "/usr/lib/haxe/std/haxe/io/Input.hx"
		return n;
	}
	
	
	public int readUInt16()
	{
		//line 238 "/usr/lib/haxe/std/haxe/io/Input.hx"
		int ch1 = this.readByte();
		//line 239 "/usr/lib/haxe/std/haxe/io/Input.hx"
		int ch2 = this.readByte();
		//line 240 "/usr/lib/haxe/std/haxe/io/Input.hx"
		if (this.bigEndian) 
		{
			//line 240 "/usr/lib/haxe/std/haxe/io/Input.hx"
			return ( ch2 | ( ch1 << 8 ) );
		}
		else
		{
			//line 240 "/usr/lib/haxe/std/haxe/io/Input.hx"
			return ( ch1 | ( ch2 << 8 ) );
		}
		
	}
	
	
	public int readInt24()
	{
		//line 249 "/usr/lib/haxe/std/haxe/io/Input.hx"
		int ch1 = this.readByte();
		//line 250 "/usr/lib/haxe/std/haxe/io/Input.hx"
		int ch2 = this.readByte();
		//line 251 "/usr/lib/haxe/std/haxe/io/Input.hx"
		int ch3 = this.readByte();
		//line 252 "/usr/lib/haxe/std/haxe/io/Input.hx"
		int n = ( (this.bigEndian) ? (( ( ch3 | ( ch2 << 8 ) ) | ( ch1 << 16 ) )) : (( ( ch1 | ( ch2 << 8 ) ) | ( ch3 << 16 ) )) );
		//line 253 "/usr/lib/haxe/std/haxe/io/Input.hx"
		if (( (( n & 8388608 )) != 0 )) 
		{
			//line 254 "/usr/lib/haxe/std/haxe/io/Input.hx"
			return ( n - 16777216 );
		}
		
		//line 255 "/usr/lib/haxe/std/haxe/io/Input.hx"
		return n;
	}
	
	
	public int readUInt24()
	{
		//line 264 "/usr/lib/haxe/std/haxe/io/Input.hx"
		int ch1 = this.readByte();
		//line 265 "/usr/lib/haxe/std/haxe/io/Input.hx"
		int ch2 = this.readByte();
		//line 266 "/usr/lib/haxe/std/haxe/io/Input.hx"
		int ch3 = this.readByte();
		//line 267 "/usr/lib/haxe/std/haxe/io/Input.hx"
		if (this.bigEndian) 
		{
			//line 267 "/usr/lib/haxe/std/haxe/io/Input.hx"
			return ( ( ch3 | ( ch2 << 8 ) ) | ( ch1 << 16 ) );
		}
		else
		{
			//line 267 "/usr/lib/haxe/std/haxe/io/Input.hx"
			return ( ( ch1 | ( ch2 << 8 ) ) | ( ch3 << 16 ) );
		}
		
	}
	
	
	public int readInt32()
	{
		//line 276 "/usr/lib/haxe/std/haxe/io/Input.hx"
		int ch1 = this.readByte();
		//line 277 "/usr/lib/haxe/std/haxe/io/Input.hx"
		int ch2 = this.readByte();
		//line 278 "/usr/lib/haxe/std/haxe/io/Input.hx"
		int ch3 = this.readByte();
		//line 279 "/usr/lib/haxe/std/haxe/io/Input.hx"
		int ch4 = this.readByte();
		//line 292 "/usr/lib/haxe/std/haxe/io/Input.hx"
		if (this.bigEndian) 
		{
			//line 292 "/usr/lib/haxe/std/haxe/io/Input.hx"
			return ( ( ( ch4 | ( ch3 << 8 ) ) | ( ch2 << 16 ) ) | ( ch1 << 24 ) );
		}
		else
		{
			//line 292 "/usr/lib/haxe/std/haxe/io/Input.hx"
			return ( ( ( ch1 | ( ch2 << 8 ) ) | ( ch3 << 16 ) ) | ( ch4 << 24 ) );
		}
		
	}
	
	
	public java.lang.String readString(int len)
	{
		//line 300 "/usr/lib/haxe/std/haxe/io/Input.hx"
		haxe.io.Bytes b = haxe.io.Bytes.alloc(len);
		//line 301 "/usr/lib/haxe/std/haxe/io/Input.hx"
		this.readFullBytes(b, 0, len);
		//line 305 "/usr/lib/haxe/std/haxe/io/Input.hx"
		return b.toString();
	}
	
	
	@Override public java.lang.Object __hx_setField(java.lang.String field, java.lang.Object value, boolean handleProperties)
	{
		//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
		{
			//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
			boolean __temp_executeDef1 = true;
			//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
			switch (field.hashCode())
			{
				case -1220931666:
				{
					//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
					if (field.equals("helper")) 
					{
						//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
						__temp_executeDef1 = false;
						//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
						this.helper = ((java.nio.ByteBuffer) (value) );
						//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
						return value;
					}
					
					//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
					break;
				}
				
				
				case 573260987:
				{
					//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
					if (field.equals("bigEndian")) 
					{
						//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
						__temp_executeDef1 = false;
						//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
						if (handleProperties) 
						{
							//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
							this.set_bigEndian(haxe.lang.Runtime.toBool(value));
							//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
							return value;
						}
						else
						{
							//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
							this.bigEndian = haxe.lang.Runtime.toBool(value);
							//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
							return value;
						}
						
					}
					
					//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
					break;
				}
				
				
			}
			
			//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
			if (__temp_executeDef1) 
			{
				//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
				return super.__hx_setField(field, value, handleProperties);
			}
			else
			{
				//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
				throw null;
			}
			
		}
		
	}
	
	
	@Override public java.lang.Object __hx_getField(java.lang.String field, boolean throwErrors, boolean isCheck, boolean handleProperties)
	{
		//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
		{
			//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
			boolean __temp_executeDef1 = true;
			//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
			switch (field.hashCode())
			{
				case -500196025:
				{
					//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
					if (field.equals("readString")) 
					{
						//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
						__temp_executeDef1 = false;
						//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
						return ((haxe.lang.Function) (new haxe.lang.Closure(this, "readString")) );
					}
					
					//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
					break;
				}
				
				
				case 573260987:
				{
					//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
					if (field.equals("bigEndian")) 
					{
						//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
						__temp_executeDef1 = false;
						//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
						return this.bigEndian;
					}
					
					//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
					break;
				}
				
				
				case -1133927784:
				{
					//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
					if (field.equals("readInt32")) 
					{
						//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
						__temp_executeDef1 = false;
						//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
						return ((haxe.lang.Function) (new haxe.lang.Closure(this, "readInt32")) );
					}
					
					//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
					break;
				}
				
				
				case -1220931666:
				{
					//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
					if (field.equals("helper")) 
					{
						//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
						__temp_executeDef1 = false;
						//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
						return this.helper;
					}
					
					//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
					break;
				}
				
				
				case -482759630:
				{
					//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
					if (field.equals("readUInt24")) 
					{
						//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
						__temp_executeDef1 = false;
						//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
						return ((haxe.lang.Function) (new haxe.lang.Closure(this, "readUInt24")) );
					}
					
					//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
					break;
				}
				
				
				case -868060226:
				{
					//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
					if (field.equals("readByte")) 
					{
						//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
						__temp_executeDef1 = false;
						//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
						return ((haxe.lang.Function) (new haxe.lang.Closure(this, "readByte")) );
					}
					
					//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
					break;
				}
				
				
				case -1133927813:
				{
					//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
					if (field.equals("readInt24")) 
					{
						//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
						__temp_executeDef1 = false;
						//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
						return ((haxe.lang.Function) (new haxe.lang.Closure(this, "readInt24")) );
					}
					
					//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
					break;
				}
				
				
				case -1140063115:
				{
					//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
					if (field.equals("readBytes")) 
					{
						//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
						__temp_executeDef1 = false;
						//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
						return ((haxe.lang.Function) (new haxe.lang.Closure(this, "readBytes")) );
					}
					
					//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
					break;
				}
				
				
				case -482759659:
				{
					//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
					if (field.equals("readUInt16")) 
					{
						//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
						__temp_executeDef1 = false;
						//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
						return ((haxe.lang.Function) (new haxe.lang.Closure(this, "readUInt16")) );
					}
					
					//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
					break;
				}
				
				
				case 94756344:
				{
					//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
					if (field.equals("close")) 
					{
						//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
						__temp_executeDef1 = false;
						//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
						return ((haxe.lang.Function) (new haxe.lang.Closure(this, "close")) );
					}
					
					//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
					break;
				}
				
				
				case -1133927842:
				{
					//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
					if (field.equals("readInt16")) 
					{
						//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
						__temp_executeDef1 = false;
						//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
						return ((haxe.lang.Function) (new haxe.lang.Closure(this, "readInt16")) );
					}
					
					//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
					break;
				}
				
				
				case 1862232350:
				{
					//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
					if (field.equals("set_bigEndian")) 
					{
						//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
						__temp_executeDef1 = false;
						//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
						return ((haxe.lang.Function) (new haxe.lang.Closure(this, "set_bigEndian")) );
					}
					
					//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
					break;
				}
				
				
				case -867862305:
				{
					//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
					if (field.equals("readInt8")) 
					{
						//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
						__temp_executeDef1 = false;
						//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
						return ((haxe.lang.Function) (new haxe.lang.Closure(this, "readInt8")) );
					}
					
					//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
					break;
				}
				
				
				case 1080375339:
				{
					//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
					if (field.equals("readAll")) 
					{
						//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
						__temp_executeDef1 = false;
						//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
						return ((haxe.lang.Function) (new haxe.lang.Closure(this, "readAll")) );
					}
					
					//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
					break;
				}
				
				
				case -934168313:
				{
					//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
					if (field.equals("readDouble")) 
					{
						//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
						__temp_executeDef1 = false;
						//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
						return ((haxe.lang.Function) (new haxe.lang.Closure(this, "readDouble")) );
					}
					
					//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
					break;
				}
				
				
				case 682499622:
				{
					//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
					if (field.equals("readFullBytes")) 
					{
						//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
						__temp_executeDef1 = false;
						//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
						return ((haxe.lang.Function) (new haxe.lang.Closure(this, "readFullBytes")) );
					}
					
					//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
					break;
				}
				
				
				case -1136761242:
				{
					//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
					if (field.equals("readFloat")) 
					{
						//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
						__temp_executeDef1 = false;
						//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
						return ((haxe.lang.Function) (new haxe.lang.Closure(this, "readFloat")) );
					}
					
					//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
					break;
				}
				
				
				case 3496342:
				{
					//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
					if (field.equals("read")) 
					{
						//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
						__temp_executeDef1 = false;
						//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
						return ((haxe.lang.Function) (new haxe.lang.Closure(this, "read")) );
					}
					
					//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
					break;
				}
				
				
				case -867777878:
				{
					//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
					if (field.equals("readLine")) 
					{
						//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
						__temp_executeDef1 = false;
						//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
						return ((haxe.lang.Function) (new haxe.lang.Closure(this, "readLine")) );
					}
					
					//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
					break;
				}
				
				
				case -1122843800:
				{
					//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
					if (field.equals("readUntil")) 
					{
						//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
						__temp_executeDef1 = false;
						//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
						return ((haxe.lang.Function) (new haxe.lang.Closure(this, "readUntil")) );
					}
					
					//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
					break;
				}
				
				
			}
			
			//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
			if (__temp_executeDef1) 
			{
				//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
				return super.__hx_getField(field, throwErrors, isCheck, handleProperties);
			}
			else
			{
				//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
				throw null;
			}
			
		}
		
	}
	
	
	@Override public java.lang.Object __hx_invokeField(java.lang.String field, haxe.root.Array dynargs)
	{
		//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
		{
			//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
			boolean __temp_executeDef1 = true;
			//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
			switch (field.hashCode())
			{
				case -500196025:
				{
					//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
					if (field.equals("readString")) 
					{
						//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
						__temp_executeDef1 = false;
						//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
						return this.readString(((int) (haxe.lang.Runtime.toInt(dynargs.__get(0))) ));
					}
					
					//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
					break;
				}
				
				
				case -868060226:
				{
					//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
					if (field.equals("readByte")) 
					{
						//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
						__temp_executeDef1 = false;
						//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
						return this.readByte();
					}
					
					//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
					break;
				}
				
				
				case -1133927784:
				{
					//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
					if (field.equals("readInt32")) 
					{
						//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
						__temp_executeDef1 = false;
						//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
						return this.readInt32();
					}
					
					//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
					break;
				}
				
				
				case -1140063115:
				{
					//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
					if (field.equals("readBytes")) 
					{
						//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
						__temp_executeDef1 = false;
						//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
						return this.readBytes(((haxe.io.Bytes) (dynargs.__get(0)) ), ((int) (haxe.lang.Runtime.toInt(dynargs.__get(1))) ), ((int) (haxe.lang.Runtime.toInt(dynargs.__get(2))) ));
					}
					
					//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
					break;
				}
				
				
				case -482759630:
				{
					//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
					if (field.equals("readUInt24")) 
					{
						//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
						__temp_executeDef1 = false;
						//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
						return this.readUInt24();
					}
					
					//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
					break;
				}
				
				
				case 94756344:
				{
					//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
					if (field.equals("close")) 
					{
						//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
						__temp_executeDef1 = false;
						//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
						this.close();
					}
					
					//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
					break;
				}
				
				
				case -1133927813:
				{
					//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
					if (field.equals("readInt24")) 
					{
						//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
						__temp_executeDef1 = false;
						//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
						return this.readInt24();
					}
					
					//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
					break;
				}
				
				
				case 1862232350:
				{
					//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
					if (field.equals("set_bigEndian")) 
					{
						//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
						__temp_executeDef1 = false;
						//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
						return this.set_bigEndian(haxe.lang.Runtime.toBool(dynargs.__get(0)));
					}
					
					//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
					break;
				}
				
				
				case -482759659:
				{
					//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
					if (field.equals("readUInt16")) 
					{
						//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
						__temp_executeDef1 = false;
						//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
						return this.readUInt16();
					}
					
					//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
					break;
				}
				
				
				case 1080375339:
				{
					//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
					if (field.equals("readAll")) 
					{
						//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
						__temp_executeDef1 = false;
						//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
						return this.readAll(dynargs.__get(0));
					}
					
					//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
					break;
				}
				
				
				case -1133927842:
				{
					//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
					if (field.equals("readInt16")) 
					{
						//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
						__temp_executeDef1 = false;
						//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
						return this.readInt16();
					}
					
					//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
					break;
				}
				
				
				case 682499622:
				{
					//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
					if (field.equals("readFullBytes")) 
					{
						//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
						__temp_executeDef1 = false;
						//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
						this.readFullBytes(((haxe.io.Bytes) (dynargs.__get(0)) ), ((int) (haxe.lang.Runtime.toInt(dynargs.__get(1))) ), ((int) (haxe.lang.Runtime.toInt(dynargs.__get(2))) ));
					}
					
					//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
					break;
				}
				
				
				case -867862305:
				{
					//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
					if (field.equals("readInt8")) 
					{
						//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
						__temp_executeDef1 = false;
						//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
						return this.readInt8();
					}
					
					//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
					break;
				}
				
				
				case 3496342:
				{
					//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
					if (field.equals("read")) 
					{
						//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
						__temp_executeDef1 = false;
						//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
						return this.read(((int) (haxe.lang.Runtime.toInt(dynargs.__get(0))) ));
					}
					
					//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
					break;
				}
				
				
				case -934168313:
				{
					//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
					if (field.equals("readDouble")) 
					{
						//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
						__temp_executeDef1 = false;
						//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
						return this.readDouble();
					}
					
					//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
					break;
				}
				
				
				case -1122843800:
				{
					//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
					if (field.equals("readUntil")) 
					{
						//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
						__temp_executeDef1 = false;
						//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
						return this.readUntil(((int) (haxe.lang.Runtime.toInt(dynargs.__get(0))) ));
					}
					
					//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
					break;
				}
				
				
				case -1136761242:
				{
					//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
					if (field.equals("readFloat")) 
					{
						//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
						__temp_executeDef1 = false;
						//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
						return this.readFloat();
					}
					
					//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
					break;
				}
				
				
				case -867777878:
				{
					//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
					if (field.equals("readLine")) 
					{
						//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
						__temp_executeDef1 = false;
						//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
						return this.readLine();
					}
					
					//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
					break;
				}
				
				
			}
			
			//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
			if (__temp_executeDef1) 
			{
				//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
				return super.__hx_invokeField(field, dynargs);
			}
			
		}
		
		//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
		return null;
	}
	
	
	@Override public void __hx_getFields(haxe.root.Array<java.lang.String> baseArr)
	{
		//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
		baseArr.push("helper");
		//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
		baseArr.push("bigEndian");
		//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
		{
			//line 28 "/usr/lib/haxe/std/haxe/io/Input.hx"
			super.__hx_getFields(baseArr);
		}
		
	}
	
	
}


